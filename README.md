...
Note that due to restrictions imposed by the consent forms signed by the subjects, some of the data used in the paper cannot be shared. The following resources have been made available:
  ...
  * The preprocessed 3D+t fMRI time series of the COBRE sample, used in the SCHIZO analysis of real data. This dataset is scheduled for release, but pending resolution of technical issues with the upload.
