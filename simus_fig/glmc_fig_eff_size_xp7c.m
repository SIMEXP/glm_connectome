...
%% Folders
path_results = '/home/pbellec/database/glm_connectome/simu_multiscale_xp7c/';
...
for num_alpha2 = 1:length(param.alpha2)
    ...
    hold on
    jbfill(list_scale',m_eff_size+2*s_eff_size,m_eff_size-2*s_eff_size,[0 0 1],[0 0 0.25],0,0.25);
    plot(list_scale,m_eff_size);
    axis([0 max(list_scale) 0 3]);
    ha = gca;   
    set(ha,'ygrid','on')
    name_fig = sprintf('xp7c_simu_alpha2x%i_nb_subject%i_sc%i',ceil(100*opt.alpha2),opt.nb_subject,opt.scale_ref);
