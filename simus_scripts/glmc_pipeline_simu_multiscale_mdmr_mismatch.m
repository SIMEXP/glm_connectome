...
folder_out = '/gs/scratch/pbellec/glm_connectome/simu_multiscale_xp8b/';

%path_roi = '/home/pbellec/database/glm_connectome/cambridge_region_growing_05scrubb/rois';
%folder_out = '/home/pbellec/database/glm_connectome/simu_multiscale_xp8b/';
    ...
    for num_sc = 1:length(param.sc)
        for num_nsub = 1:length(param.nsub)
            for num_alpha2 = 1:length(param.alpha2)
                ...
                num_seed = num_seed+1;        
                pipe = psom_add_job(pipe,name_job,'glmc_brick_simu_cwas',in,out,opt);
            end
        end
    end
end
